...
# Class representing the crime scene
class CrimeScene:

    def __init__(self, location):
        self.location = location
        self.__investigated = False
        # Private attribute indicating investigation status

    # Getter property for 'investigated' attribute
    @property
    def investigated(self):
        return self.__investigated

    # Setter property for 'investigated' attribute
    @investigated.setter
...
# Subclass of Character representing ExBestfriend
...
class Game:
    ...
    def move_to_room(self):
        ...
        if 0 < room_choice <= len(self.__lodge_rooms):
            self.current_room = room_choice - 1  # Adjusting for 0-based indexing
            print(f"You enter the {self.__lodge_rooms[self.current_room].name}. Description:{self.__lodge_rooms[self.current_room].description}")
